  ...
  **/
  ...
  function createJsString(text) {
    const byteLength = text.length * 2
      , ptr = exports.malloc(byteLength)
      ;
    // Had an error:
    //    hbjs.mjs:303 Uncaught (in promise) RangeError: start offset of
    // Uint16Array should be a multiple of 2
    // TODO: need a minimal case to reproduce in order to file an upstream issue.
    const words = new Uint16Array(text.length);
    for (let i = 0; i < words.length; ++i) words[i] = text.charCodeAt(i);
    new Uint8Array(Module.wasmMemory.buffer, ptr, byteLength).set(new Uint8Array(words.buffer));
